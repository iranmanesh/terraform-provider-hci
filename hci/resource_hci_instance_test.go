...
const hciInstance = "hci_instance"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckInstanceCreateBasicExists("hci_instance.foobar"),
				),
			},
		},
	})
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckInstanceCreateDataDriveExists("hci_instance.foobar"),
				),
			},
		},
	})
...
resource %s "foobar" {
	...
	cpu_count        = 1
	memory_in_mb     = 1024
}`, hciInstance, environment, network, name)
}
...
resource "hci_volume" "foobar" {
	environment_id = "%s"
    name           = "%s"
	disk_offering  = "Performance, No QoS"
	size_in_gb     = "10"
    instance_id    = "${hci_instance.foobar.id}"
}`, hciInstance, environment, network, name, environment, name)
}
